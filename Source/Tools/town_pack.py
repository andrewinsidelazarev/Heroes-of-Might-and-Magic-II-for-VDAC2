...
def build_payload(palette, town_img, strip_img, names_masks, font_masks, recruit_win, monster_sprites, gold_icon, monh_sprites):
    payload = bytearray()
    ...
    return (payload, pal_addr, img_addr, strip_addr, name_pal_addr, name_addrs, font_addrs,
            recruit_addr, spr_pal_addr, mon_addrs, gold_addr, monh_addrs)


def emit_inc(pal_addr, img_addr, strip_addr, name_pal_addr, name_addrs, font_addrs, block_hit, pak,
             wrapped_descs, recruit_addr, spr_pal_addr, mon_addrs, gold_addr, monh_addrs):
    L = []
    ...
    for i in range(len(RECRUIT_NAMES)):
        c = {0: 40, 1: 4}.get(i, 0)
        L.append(f"                DW {c}")
    # --- Полный динам.гарнизон: MONH-портреты + якоря динам.слотов 2,3,4 (новые типы) ---
    L.append("RecruitMonhTab:                        ; [recruit idx] → MONH-портрет гарнизона ×1.6 [lo,mid,hi,w,h]")
    for (addr, w, h) in monh_addrs:
        L.append(f"                DEFB #{addr & 0xFF:02X}, #{(addr >> 8) & 0xFF:02X}, "
                 f"#{(addr >> 16) & 0xFF:02X}, {w}, {h}")
    gar_bottom = panel_y0 + round(99 * 1.6)          # низ ячейки армбара (экран)
    L.append(f"GAR_ANCHOR_Y         EQU {gar_bottom * 16}")
    for s in (2, 3, 4):
        cellx = 112 + s * 88
        L.append(f"GAR_ANCHOR_X{s}        EQU {round((cellx + 41) * 1.6) * 16}")   # центр ячейки X
        L.append(f"GAR_CNT{s}_VX          EQU {round((cellx + 64) * 1.6) * 16}")    # счётчик слота
    L.append("")
    L.append("                endif")
...
def main() -> int:
    ...
    (payload, pal_addr, img_addr, strip_addr, name_pal_addr, name_addrs, font_addrs,
     recruit_addr, spr_pal_addr, mon_addrs, gold_addr, monh_addrs) = build_payload(
        palette, town_img, strip_img, names_masks, font_masks, recruit_win, monster_sprites, gold_icon, monh_sprites)
    summary = build_pak(
