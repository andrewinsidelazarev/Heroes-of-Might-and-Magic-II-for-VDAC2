...
def prepare_host_package(root: Path) -> list[Path]:
    package_dir = root / "Build" / "SD" / "Games" / HMM2_FOLDER
...
def main() -> int:
    ...
    ap.add_argument("--base-img", default=r"\\tsclient\D\Работа.Андрей\unreal_x64 - HMM2\wc.img")
    ...
    hmm2 = image.ensure_dir(games, HMM2_FOLDER)
    for file in package_files:
        image.write_file(hmm2, file.name, file)
    image.save()

    print(f"image: {out_img} (in-place)")
    print(f"folder: /Games/{HMM2_FOLDER}/")
    for file in package_files:
