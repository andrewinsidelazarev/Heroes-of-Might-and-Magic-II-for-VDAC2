...
for t in range(16):
    if gm()==1: print(f"  ГОРОД gm=1 через ~{t}", flush=True); break
    time.sleep(0.5)
time.sleep(1.5)
img=Image.open(str(U/"ft812_dump.bmp")).convert("RGB"); img.save(str(DIAG/"scn_town.png"))
