...
.hplw:          LD   (ArmyInfoHpl), HL
                ...
                PUSH BC                             ; ---- стрим попапа из PAK ----
                LD   HL, BattlePakName              ; открыть HMM2BATL.PAK (имя в общий буфер)
                LD   DE, MenuNameBuf
                LD   BC, 13
                LDIR
                CALL Loader_Init
                ...
                LD   BC, ARMYINFO_SECTORS           ; стрим в область финала
...
.noatkanim:     LD   A, (BattleDeathActive)        ; идёт анимация смерти? → тикать её
                ...
                XOR  A
                RET
.noanim:        ; --- Попап ArmyInfo: ПКМ-hold (Dialog::ZERO) ИЛИ модалка с EXIT (Dialog::BUTTONS) ---
                LD   A, (ArmyInfoShow)
                OR   A
                JR   Z, .no_popup_held
                LD   A, (ArmyInfoModal)
...
; → FT_VERTEX2F. Смещения per-кадр (ox/oy из ICN, reflect учтён в таблице). Портит A,BC,DE,HL.
...
; C=группа → A=длина последовательности (0 если группы нет). Портит A,DE,HL (C цел).
Battle_SeqLen:
                CALL Battle_SeqAddr
                LD   A, 0
                RET  Z
...
; Нет группы → слот стойки типа. Портит A,DE,HL.
Battle_SeqSlot:
