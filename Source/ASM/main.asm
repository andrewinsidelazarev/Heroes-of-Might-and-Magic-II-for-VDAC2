...
AI_KINGDOM_HEROX_OFS EQU AI_KINGDOM_OFS + 2       ; Sorc герой: тайл X (1б)
AI_KINGDOM_HEROY_OFS EQU AI_KINGDOM_OFS + 3       ; Sorc герой: тайл Y (1б)
AI_KINGDOM_HAS_HERO_OFS EQU AI_KINGDOM_OFS + 4    ; 1=Sorc герой на карте, 0=нет
AI_KINGDOM_LEN       EQU 5                          ; gold(2)+heroX+heroY+hasHero (нарастает: армия)
...
AiKingdom_Init:
                ...
                LD   HL, GLOBAL_STATE_BASE + AI_KINGDOM_HEROX_OFS
                LD   (HL), AI_SORC_HERO_START_X    ; Sorc герой стартовая позиция (рядом с замком)
                INC  HL
                LD   (HL), AI_SORC_HERO_START_Y
                INC  HL
                LD   (HL), 1                       ; hasHero = 1 (Sorc герой на карте)
                POP  AF
                SetPage3_A
                RET
