...
Init_Video:     ; --- 1. Sanity-check: VDAC2 присутствует на плате? --------
                ...
                ; --- 1b. Shared SPI (#77/#57) → IDLE (как Zuma BootFtBegin) ---
                ; ВРЕМЕННО ОТКЛЮЧЕНО (бинарный поиск регресса «белый экран на реале»):
                ; после добавления этого блока на реале меню белое (loader-стрим с SD
                ; не приносит ассеты) — проверяем, виноват ли idle-код.
                if 0
                LD   A, #03
                LD   BC, #0077
                OUT  (C), A
