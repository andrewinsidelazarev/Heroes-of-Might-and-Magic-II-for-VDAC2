...
**Часть II. Журнал разработки Zuma (главы 12–35)** — практический опыт в
...
- **Музыка через MIDI:** 35 (GM-синтезатор SAM2695 на порту AY — bit-bang 31250 бод на рег 14 бит 2, рег 7 = port A на выход, GM System On; пайплайн XMI→SMF→покадровый поток; покадровый Z80-плеер; два корня тишины — порт не на выход и неточный тайминг).

> Нумерация глав сквозная (1→35). Историческое примечание: ранние версии
...
## Глава 35. Музыка через MIDI-синтезатор SAM2695 на порту AY (2026-06-17)

Звук на ZX-Evo делится на ДВА независимых тракта, и их нельзя путать:

- **General Sound (MultiSound)** — семплерный сопроцессор (MOD/WAV/MP3-движок). Про него глава 34. Годится под **SFX** (клики, взрывы) и потоковую музыку из семплов.
- **SAM2695** (Dream) — аппаратный **General MIDI**-синтезатор. Принимает стандартный MIDI-поток и сам синтезирует ноты по GM-инструментам. На ZX-Evo его data-вход подключён к **MIDI-порту AY** (как у `zx-midiplayer` UzixLS). Это идеальный тракт для **музыки**, у которой оригинал — MIDI (как у Heroes of Might and Magic II: треки лежат в `HEROES2.AGG` как XMI).

Эта глава — как заиграть MIDI-музыку через SAM2695. mp3/ogg тут не нужны и невозможны: источник — MIDI, синтез — в чипе.

### 35.1. Где физически MIDI-выход

MIDI шлётся через **AY-3-891x, регистр 14 (I/O port A), бит 2** — стандартная разводка ZX Spectrum 128 (RS232 `RXD`). Скорость — стандартный MIDI **31250 бод**, формат UART: 1 старт-бит (0) + 8 бит данных (LSB вперёд) + 1 стоп-бит (1). Никакого аппаратного UART нет — линия дёргается программно (bit-bang) записью в порт A.

Запись в AY: порт `#FFFD` выбирает регистр, `#BFFD` пишет данные. Значение порта A: бит2=1 → `#FE`, бит2=0 → `#FA` (прочие биты держим в idle-high, чтобы не дёргать линию `CTS` клавиатуры).

### 35.2. Инициализация порта — БЕЗ неё тишина

**Самый частый баг (стоил полного «нет звука»): port A не переведён в режим ВЫХОДА.** По умолчанию порт A — вход; запись в регистр 14 тогда никуда не идёт, на пин ничего не выходит. Перед любой отправкой надо в **регистре 7** (mixer/направление портов) поднять **бит 6** (`#FC`):

```asm
Music_InitPort:
        ld   bc,#fffd
        ld   a,#07          ; AY рег 7 — направление портов
        out  (c),a
        ld   b,#bf          ; bc=#BFFD
        ld   a,#fc          ; бит6=1 → port A на ВЫХОД
        out  (c),a
        ld   b,#ff
        ld   a,#0e          ; рег 14
        out  (c),a
        ld   b,#bf
        ld   a,#fe          ; бит2=1 → линия idle-high
        out  (c),a
        ret
```

(У `zx-midiplayer` это `uart_prepare`. Там же — опциональный TurboSound chip-select: `out #FFFD,#FF`/`#FE` выбирает AY0/AY1, если MIDI-линия на конкретном чипе; на одиночном AY не нужен.)

### 35.3. Отправка байта (bit-bang 31250) — точный тайминг решает всё

Второй баг тишины: **неточная скорость**. UART терпит ±~5 %; если bit-period мимо — SAM2695 молча отбраковывает байты. Нужен **точный период бита под частоту CPU**. На TS-Conf turbo **14 МГц** один бит = **448 тактов**. Калибровку проще взять 1:1 из проверенного `zx-midiplayer uart_putc`:

```asm
; A = байт. Порт A рег14 бит2, 31250 бод @14МГц.
MIDI_PutByte:
        ld   e,a
        ld   bc,#fffd : ld a,#0e : out (c),a   ; выбрать рег 14
        ld   b,#bf                              ; bc=#BFFD
        ld   d,1+8+1                            ; START+8DATA+STOP
        scf                                     ; STOP-бит в CF (выдвинется через RR E)
        jr   .put0                              ; START (0)
.loop   rr   e            ; (8)  следующий бит (LSB) в CF
        jp   nc,.put0     ; (10)
.put1   ld   a,#fe : out (c),a : jr .nextbit    ; (7+11+12) бит2=1
.put0   ld   a,#fa : out (c),a : jr .nextbit    ; (7+11+12) бит2=0 — СИММЕТРИЧНО put1
.nextbit
        ld   a,0          ; (7)  .A — тайминг-наполнитель
        ld   a,26         ; (7)  .B = 26 (для 14 МГц)
.dlb    dec  a : jp nz,.dlb   ; (14·26)
        nop : nop         ; (8)
        dec  d : jp nz,.loop  ; (4+10) → ровно 448 тактов/бит
        ld   e,24         ; .C = 24 — зазор после стоп-бита (14 МГц)
.dlc    dec  e : jr nz,.dlc
        ret
```

Тонкости, без которых «плывёт»/молчит:

- **Симметрия 0/1.** Обе ветки `put_0`/`put_1` должны иметь одинаковый путь (обе `jr .nextbit`). Если у одной `jr`, а другая «проваливается» — биты 0 и 1 разной длины → jitter → брак.
- **`JP`, не `JR`** в задержке: у них разный тайминг (10 vs 12 тактов); калибровка в тактах считалась под `JP`.
- **Две задержки**: `.B` (внутри бита) и `.C` (после стоп-бита, межбайтовый зазор). Одной мало.

Константы `.B/.C` по частотам (из `uart_init`, самомодифицирующийся код): 3.5 МГц → ~2 / 6; 7 МГц → 10 / 12; **14 МГц → 26 / 24**; 28 МГц → 58 / 48. Частоту берём фиксировано под целевую сборку (у нас 14 МГц).

**Про `DI`/`EI`.** Оригинал делает `di` на время байта: строчный INT 15625 Гц (см. главу про прерывания) приходит чаще, чем длятся 2 бита, и его ISR разрушит тайминг. **Но** если в вашем порте прерывания уже выключены глобально (у нас `platform_tsconf`: `DI` + маска INT = 0, рендер на поллинге) — `di/ei` НЕ нужны, и `ei` даже вреден (включит то, что вы намеренно отключили). Сверьтесь со своим состоянием прерываний.

### 35.4. General-MIDI инициализация и «паника»

SAM2695 переводят в режим GM одним SysEx **GM System On** (раз при старте). При остановке трека — гасим все ноты на 16 каналах (`CC123 All Notes Off`), иначе зависшие ноты «тянутся»:

```asm
Music_GMReset:           ; F0 7E 7F 09 01 F7
        ld   hl,MidiGMOn : ld b,6
.l      ld a,(hl) : push hl : push bc : call MIDI_PutByte : pop bc : pop hl : inc hl : djnz .l
        ret
MidiGMOn db #f0,#7e,#7f,#09,#01,#f7

Music_Stop:              ; CC123=0 на каналах 0..15
        ld   b,0
.ch     push bc
        ld a,#b0 : or b : call MIDI_PutByte    ; status = ControlChange|канал
        ld a,#7b : call MIDI_PutByte           ; CC 123
        xor a    : call MIDI_PutByte
        pop  bc : inc b : ld a,b : cp 16 : jr c,.ch
        ret
```

### 35.5. Источник данных: XMI → SMF → покадровый поток

Оригинальная музыка (HMM2) — **XMI** (Miles/AIL) внутри `*.AGG`. Пайплайн (host-сторона, Python):

1. Извлечь XMI из AGG (`agg_tools.py --extract-xmi`).
2. **XMI → стандартный MIDI (SMF type 0)** — `xmi2mid.py`, точный порт `fheroes2/engine/audio_xmi2mid.cpp`. Ключевые отличия XMI от SMF: задержка в XMI — это сумма подряд идущих байт < 128 (не VLQ); у `note-on` нет парного `note-off` — после 2 байт идёт VLQ-длительность, из неё генерируется `note-off` на `time+dur`; все события собираются с абсолютным временем, сортируются, затем дельты пакуются VLQ; `ppqn = tempo·3/25000`. **Полезно: SMF можно проиграть на ПК любым MIDI-плеером и убедиться, что данные верны — это отделяет «кривой конвертер» от «кривого Z80-вывода».**
3. **SMF → компактный ПОКАДРОВЫЙ поток** — `music_pack.py`. Вся музыкальная арифметика (ppqn, tempo, тики → время) делается ЗДЕСЬ, чтобы Z80-плеер остался тривиальным. Токены потока: `#F8 nn` — ждать nn кадров; `#FF` — конец (зациклить); `#80..#EF …` — MIDI-событие (длина данных по нибблу status: `Cx/Dx`→1, иначе 2; без running status). Темп кадра = частота FT812 (59 Гц).

### 35.6. Z80-плеер: один тик за кадр

Поток лежит в overlay-странице (slot3 `#C000`), резидентный плеер её читает. `Music_Tick` зовётся раз за кадр из главного апдейта: отсчитывает паузы, на нуле — шлёт все события до следующей паузы.

```asm
Music_Tick:
        ld a,(MusicActive) : or a : ret z
        ld a,(MusicWait)   : or a : jr z,.play
        dec a : ld (MusicWait),a : ret          ; ждём ещё кадр
.play   ld hl,(MusicPtr)
.next   ld a,(hl)
        cp #f8 : jr z,.wait                      ; WAIT nn
        cp #ff : jr z,.loop                      ; END → начало
        and #f0 : cp #c0 : jr z,.s1 : cp #d0 : jr z,.s1
.s2     ld a,(hl):call MIDI_PutByte:inc hl       ; status+2 данных
        ld a,(hl):call MIDI_PutByte:inc hl
        ld a,(hl):call MIDI_PutByte:inc hl : jr .next
.s1     ld a,(hl):call MIDI_PutByte:inc hl       ; status+1 данных
        ld a,(hl):call MIDI_PutByte:inc hl : jr .next
.wait   inc hl : ld a,(hl) : ld (MusicWait),a : inc hl : ld (MusicPtr),hl : ret
.loop   ld hl,(MusicStart) : ld (MusicPtr),hl : ret
```

Hooks сцен: `Game_Init` → `Music_InitPort` + `Music_GMReset`; вход в экран с музыкой → выставить slot3 на страницу потока + `Music_Start`; апдейт сцены → `Music_Tick`; выход из сцены → `Music_Stop` (slot3 уходит под другое).

### 35.7. Проверка без слуха и уроки

- **Эмулятор не синтезирует звук**, но поток можно проверить логически: перехватить запись AY (`out #FFFD`/`#BFFD`) и **декодировать UART обратно в MIDI-байты** (старт→8 бит→стоп). Так в `tsconf_ft812_sim` тест убеждается, что `Music_GMReset` шлёт ровно `F0 7E 7F 09 01 F7`, а плеер выдаёт события трека. **Важно: декодировать только если port A реально настроен на выход (рег7 бит6)** — иначе тест не поймает пропуск `Music_InitPort` и даст ложное «играет».
- **«Нет звука» на железе = почти всегда одно из двух**: (1) port A не на выход (рег 7), (2) bit-period мимо 31250. И то, и другое не видно в наивном эмуляторе — оба чисто железные (родня уроку главы 34).
- **Данные ≠ вывод.** Если MID-файл играет на ПК, а на железе тишина — конвертер ни при чём, копайте Z80-вывод/тайминг.
- **GS и SAM2695 — разные подсистемы.** SFX можно оставить на GS, музыку вести через MIDI; они не конфликтуют.
- После правок ASM не забывать перезаписать образ (SPG на SD), иначе на железе старая версия (родня уроку про обновление образа).
