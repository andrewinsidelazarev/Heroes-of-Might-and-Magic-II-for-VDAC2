...
void wm(unsigned addr, u8 val)
{
  ...
  if (conf.mem_model == MM_TSL)
  {
   ...
   if (::debug_w0trace_frames > 0 && conf.mem_model == MM_TSL && addr >= 0x3800 && addr < 0x4000)
   {
      const unsigned window = (addr >> 14) & 3;
      const unsigned blocked = (window == 0 && !comp.ts.vdos && !comp.ts.w0_we) ? 1 : 0;
      ::debug_w0trace_log(blocked ? "wm-3800-blocked" : "wm-3800", addr, val, comp.ts.page[window], blocked);
   }
