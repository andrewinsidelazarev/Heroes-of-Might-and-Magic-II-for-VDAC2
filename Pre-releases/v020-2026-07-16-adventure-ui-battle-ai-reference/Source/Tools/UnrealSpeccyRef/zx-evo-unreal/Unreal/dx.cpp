...
void readmouse(DIMOUSESTATE *md)
{
   ...
   readdevice(md, sizeof *md, dimouse);   // кнопки/колесо (по RDP надёжны)
   // RDP-fix: относительные дельты DirectInput по RDP рвутся (RDP отдаёт мышь абсолютно).
   // Берём дельту АБСОЛЮТНОЙ позиции GetCursorPos (по RDP надёжна), БЕЗ рецентра
   // (SetCursorPos по RDP лагает и сам дёргает). Кнопки/колесо — от DirectInput.
   // Локально (не RDP) поведение не меняем.
   if (active && GetSystemMetrics(SM_REMOTESESSION))
   {
      // RDP: хостовый курсор АБСОЛЮТНЫЙ и зажат в окно (ClipCursor, dx.cpp:501). Отдаём
      // абсолютную позицию прямо как счётчик (input.cpp:366 msx=md.lX, msy=-md.lY); игра
      // берёт РАЗНОСТЬ соседних Kempston-чтений → игровой курсор следует за хостовым 1:1,
      // дрейфа НЕТ (нет накопления/рецентра, нечему уезжать).
      //
      // Бесконечное относительное движение (как реальная Kempston-мышь, через рецентр
      // SetCursorPos в центр окна) по этому RDP НЕВОЗМОЖНО: доказано mouselog — SetCursorPos
      // НЕ доезжает (RDP-клиент непрерывно перетирает серверный курсор абсолютной позицией
      // физической мыши), off рос до 5161 (дрейф), rc держался 47% кадров (запирание). Любая
      // схема с рецентром по RDP обречена. Поэтому — чистый absolute.
      //
      // Минус absolute: у самой границы окна хостовому курсору некуда → игровой может
      // упереться (для point-and-click ок: край достаётся прометанием через окно, игра
      // клампит). Бесконечное движение — только опц. edge-push (эвристика), если попросят.
      POINT pt;
      if (GetCursorPos(&pt))
      {
         md->lX = pt.x; md->lY = pt.y;
         // диагностика по флагу mlog.on
         FILE *lf = fopen("mlog.on", "rb");
         if (lf) { fclose(lf); FILE *o = fopen("mouselog.txt", "a");
            if (o) { fprintf(o, "pt %ld,%ld\n", pt.x, pt.y); fclose(o); } }
      }
   }
}
