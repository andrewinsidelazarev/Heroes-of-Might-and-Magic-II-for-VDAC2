...
extern int debug_w0trace_frames;
...
void ts_ext_port_wr(u8 port, u8 val)
{
  switch (port)
  {
    ...
    // memory
    case TSW_MEMCONF:
      debug_w0trace_log("out-memconf-before", 0x21AF, val, comp.ts.memconf, 0);
      comp.ts.memconf = val;
      comp.p7FFD &= ~0x10;
      comp.p7FFD |= (val & 1) << 4;
      set_banks();
      debug_w0trace_log("out-memconf-after", 0x21AF, val, comp.ts.memconf, 0);
    break;

    case TSW_PAGE0:
      comp.ts.page[0] = val;
      set_banks();
      debug_w0trace_log("out-page0", 0x10AF, val, comp.ts.memconf, 0);
    break;

    case TSW_PAGE1:
      comp.ts.page[1] = val;
      set_banks();
      debug_w0trace_log("out-page1", 0x11AF, val, comp.ts.memconf, 0);
    break;

    case TSW_PAGE2:
      comp.ts.page[2] = val;
      set_banks();
      debug_w0trace_log("out-page2", 0x12AF, val, comp.ts.memconf, 0);
    break;

    case TSW_PAGE3:
      comp.ts.page[3] = val;
      set_banks();
      debug_w0trace_log("out-page3", 0x13AF, val, comp.ts.memconf, 0);
    break;
